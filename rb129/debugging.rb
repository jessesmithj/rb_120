...
# OR community_library.books.each(&:display_data)
...
#   def ==(other)
#     serial_number == other.serial_number #cannot use an explicit caller with private, only works within the class, not for instances of the class
#   end
...
# You started writing a very basic class for handling files. However, when you begin 
# to write some simple test code, you get a NameError. The error message complains 
# of an uninitialized constant File::FORMAT.
...
#   # def to_s
#   #   "#{name}.#{self.class::FORMAT}" # one possible solution is to keep the CONSTANTS in each subclass and use this method instead
#   # end
# end
...
# puts FooBar.new.foo # => "foo"
# puts FooBar.new.baz # => "foo"
...
# However, we are surprised by what we see when we test its behavior. Why are we seeing this unexpected output? 
...
#   def display_name
#     puts "The student's name is #{@name}."
#   end
# end

# jesse = Student.new('Jesse')
# jesse.display_name  #=> outputs "The student's name is Jesse."
